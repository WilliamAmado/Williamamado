...
function traerInformacionClientes(){
    $.ajax({
        url:"http://193.122.181.170:8080/api/Client/all", //colocar la http del modulo de la tabla CLIENT
        type:"GET",
        datatype:"JSON",
...
function pintarRespuestaClient(respuestaC){
    ...
    for(i=0; i<respuestaC.length; i++){
        ...
        <td><button type="submit" class="btn btn-info btn-lg btn-danger" onclick="borrarElementoClient(${respuestaC[i].idClient})"><span class="glyphicon glyphicon-trash"></span>Borrar</button>
        </td>`;
    }
    myTable+="</table>";
...
function guardarElementoClient(){
    ...
    $.ajax({
        ...
        url:"http://193.122.181.170:8080/api/Client/save",
...
function actualizarElementoClient(idElementoC){
    ...
    $.ajax({
        url:"http://193.122.181.170:8080/api/Client/update", //colocar la http del modulo de la tabla CLIENT
        type:"PUT",
        data:dataToSend,
...
function borrarElementoClient(idElementoC){
    ...
    let dataToSend=JSON.stringify(myDataC);
    $.ajax({
        url:"http://193.122.181.170:8080/api/Client/"+idElementoC,
        type:"DELETE",
        data:dataToSend,
        contentType:"application/JSON",
        datatype:"JSON",
