...
$(document).ready(function (){
    ...
    $.get("http://193.122.181.170:8080/user",function(data){
...
function traerInformacionCategory(){
    $.ajax({
        url:"http://193.122.181.170:8080/api/Category/all",
        type:"GET",
        datatype:"JSON",
...
function pintarRespuestaCategory(respuestaCategory){
    ...
    for(i=0; i<respuestaCategory.length; i++){
        ...
        <td><button type="submit" class="btn btn-danger" onclick="borrarElementoCategory(${respuestaCategory[i].id})"><span class="glyphicon glyphicon-trash"></span>Borrar</button>
        </tr>`;
    }
    myTable+="</table>";
...
function guardarElementoCategory(){
    ...
    $.ajax({
        ...
        url:"http://193.122.181.170:8080/api/Category/save",
        success:function(responseC) {
            console.log(responseC);
            console.log("Se guardo correctamente");
            alert("Categoria se Guardo Correctamente");
            window.location.reload();
        },
        error: function(jqXHR, textStatus, errorThrown) {
            window.location.reload();
            alert("Categoria no se guardo correctamente");
        }
...
function actualizarElementoCategory(idElemento){
    ...
    let dataToSend=JSON.stringify(myData);
    $.ajax({
        url:"http://193.122.181.170:8080/api/Category/update", //colocar la http del modulo de la tabla CLIENT
        type:"PUT",
        data:dataToSend,
...
function borrarElementoCategory(idElemento){
    ...
    let dataToSend=JSON.stringify(myData);
    $.ajax({
        url:"http://193.122.181.170:8080/api/Category/"+idElemento,
        type:"DELETE",
        data:dataToSend,
        contentType:"application/JSON",
        datatype:"JSON",
