...
function traerInformacionC(){
    $.ajax({
        url:"http://193.122.181.170:8080/api/Category/all",  //"http://150.230.95.100:8080/api/Category/all"
        type:"GET",
        datatype:"JSON",
...
//Funcion que trae la informacion de Motorbike
function traerInformacionMotorbike(){
    $.ajax({
        url:"http://193.122.181.170:8080/api/Motorbike/all",
        type:"GET",
        datatype:"JSON",
...
function pintarRespuestaMotorbike(respuestaMotorbike){
    ...
    for(i=0; i<respuestaMotorbike.length; i++){
        myTable +=`
        <div class="card text-black card border-danger  mb-3" style="width: 18rem;">
            <div class="card-body">
...
function guardarElementoMotorbike(){
    ...
    $.ajax({
        ...
        url:"http://193.122.181.170:8080/api/Motorbike/save",
...
function actualizarElementoMotorbike(idElemento){
    ...
    $.ajax({
        url:"http://193.122.181.170:8080/api/Motorbike/update", //colocar la http del modulo de la tabla CLIENT
        type:"PUT",
        data:dataToSend,
...
function borrarElementoMotorbike(idElemento){
    ...
    let dataToSend=JSON.stringify(myData);
    $.ajax({
        url:"http://193.122.181.170:8080/api/Motorbike/"+idElemento,
        type:"DELETE",
        data:dataToSend,
        contentType:"application/JSON",
        datatype:"JSON",
